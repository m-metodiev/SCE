...
test = calc_tilde_G_inv(matList2$Ml[[1]],matList2$Al[[1]],beta)
diag(test)=0
...
source("functions/cov_TFR_fit_funcs.R")
fit_param(dimension, num_observations, matList2, sim2, Sigma=Sigma,id_min=1:dim(matList2$Fk[[1]])[1],
...
fit1_func = function(sim) fit_param(dimension, num_observations, matList2, sim, id_min=1:dim(matList2$Fk[[1]])[1],
                                    filename_error_measures=TRUE,save=FALSE,Sigma=Sigma, compute_WSCE = TRUE)
fit2_func = function(sim) fit_param(dimension, num_observations, matList2, sim, id_min=1:dim(matList2$Fk[[1]])[1],
                                    filename_error_measures = TRUE,
                                    save=FALSE,Sigma=Sigma,link=combined_matList, link_der_beta = link_der_combined,
                                    compute_WSCE=TRUE)
