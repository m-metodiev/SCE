...
dimension <- 200; num_observations = 10; beta = .982

matList2 = sim_matList(dimension,beta=beta,num_F=2,k_vec=c(3,10),num_G=1,F_0=FALSE)
matList2$Fk[[length(matList2$Fk)+1]] = matrix(1,nrow=dimension,ncol=dimension)
...
test = tilde_G_inv(matList2$Ml[[1]],matList2$Al[[1]],beta)
diag(test)=0
...
sim2 = sim_cov(num_observations, Sigma)
names(matList2$Fk) = c("comcol","sameRegion","intercept")
## END Initialization ##

## BEGIN one simulation ##
source("functions/cov_TFR_fit_funcs.R")
fit_param(dimension, num_observations, matList2, sim2, Sigma=Sigma,adj_positions=1:dim(matList2$Fk[[1]])[1],
          ...
          #interaction_effects=list(c("comcol","sameRegion"),c("comcol","spatial")),
          compute_WSCE = TRUE
          )
...
fit_param(dimension, num_observations, matList2, sim2, Sigma=Sigma,adj_positions=1:dim(matList2$Fk[[1]])[1],
          ...
          normalize_data = TRUE, compute_WSCE = TRUE, joint_estimation = TRUE)
...
set.seed(seed)
num_sim=40
sim_func = function(seed) sim_cov_with_seed(num_observations, Sigma, seed)
fit1_func = function(sim) fit_param(dimension, num_observations, matList2, sim, adj_positions=1:dim(matList2$Fk[[1]])[1],
                                    filename_error_measures=TRUE,save=FALSE,Sigma=Sigma, compute_WSCE = TRUE)
fit2_func = function(sim) fit_param(dimension, num_observations, matList2, sim, adj_positions=1:dim(matList2$Fk[[1]])[1],
                                    filename_error_measures = TRUE,
                                    save=FALSE,Sigma=Sigma,
...
## BEGIN many simulations for mu and sigma unknown (2 step)##
set.seed(seed)
num_sim=40
sim_func = function(seed) sim_cov_with_seed(num_observations, Sigma, seed)
fit1_func = function(sim) fit_param(dimension, num_observations, matList2, sim, adj_positions=1:dim(matList2$Fk[[1]])[1],
...
fit2_func = function(sim) fit_param(dimension, num_observations, matList2, sim, 
                                    adj_positions=1:dim(matList2$Fk[[1]])[1],
                                    filename_error_measures = TRUE,
                                    save=FALSE,Sigma=Sigma,
                                    interaction_effects=list(c("comcol","sameRegion"),
                                                             c("comcol","spatial"),
...
## END many simulations for mu and sigma unknown ##

## BEGIN many simulations for mu and sigma unknown (1 step)##
set.seed(seed)
num_sim=40
sim_func = function(seed) sim_cov_with_seed(num_observations, Sigma, seed)
fit1_func = function(sim) fit_param(dimension, num_observations, matList2, sim, adj_positions=1:dim(matList2$Fk[[1]])[1],
...
fit2_func = function(sim) fit_param(dimension, num_observations, matList2, sim, 
                                    adj_positions=1:dim(matList2$Fk[[1]])[1],
                                    filename_error_measures = TRUE,
                                    save=FALSE,Sigma=Sigma, # warning: the interaction effect was removed to save computation time
                                    compute_WSCE = TRUE, normalize_data = TRUE,
                                    joint_estimation=TRUE)
sims_errors_and_bic = sim_errors_and_bic(sim_func,fit1_func,fit2_func,num_sim)
write.csv(sims_errors_and_bic,
          file=paste(data_source,"sim_01_sims_errors_and_bic_musigma_unknown_1step.csv",sep=""))
## END many simulations for mu and sigma unknown ##